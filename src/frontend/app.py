...
if page == "Dashboard":
    ...
    st.markdown(
        '<h1 class="main-header">📸 Visual Inventory Audit</h1>', unsafe_allow_html=True
    )

    st.write(
        "Upload a photo of your medical store shelf to automatically detect medicines and stock levels."
    )

    uploaded_file = st.file_uploader(
        "Choose a shelf image", type=["jpg", "jpeg", "png"]
    )
    ...
    if uploaded_file is not None:
        ...
        with col1:
            st.image(
                uploaded_file, caption="Uploaded Shelf Image", use_container_width=True
            )
                    ...
                    if result:
                        ...
                        for med in medicines:
                            with st.expander(f"💊 {med['name']} - {med['brand']}"):
                                ...
                                st.write(
                                    f"**Confidence:** {med.get('confidence_score', 0.9) * 100:.1f}%"
                                )
                                if med.get("expiry_visible"):
                                    st.write(
                                        f"**Expiry:** {med.get('expiry_date', 'N/A')}"
                                    )
                        ...
                        if missing:
                            st.warning(
                                f"⚠️ Missing Essential Medicines: {', '.join(missing)}"
                            )

elif page == "Expiry Monitor":
    st.markdown(
        '<h1 class="main-header">⏰ Expiry Date Monitor</h1>', unsafe_allow_html=True
    )
    ...
    st.markdown(
        '<h1 class="main-header">📝 Prescription to Order</h1>', unsafe_allow_html=True
    )

    st.write(
        "Upload a handwritten prescription to automatically generate a purchase order."
    )

    uploaded_file = st.file_uploader(
        "Choose a prescription image", type=["jpg", "jpeg", "png"]
    )
    ...
    if uploaded_file is not None:
        ...
        with col1:
            st.image(
                uploaded_file, caption="Uploaded Prescription", use_container_width=True
            )
                ...
                with st.spinner("Extracting medicines from prescription..."):
                    ...
                    if result:
                        ...
                        for med in medicines:
                            ...
                            st.write(
                                f"   Quantity: {med['quantity']} | Frequency: {med.get('frequency', 'N/A')}"
                            )
    ...
    st.markdown(
        '<h1 class="main-header">🛡️ Drug Safety Checker</h1>', unsafe_allow_html=True
    )
